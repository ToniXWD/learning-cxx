...
int main(int argc, char **argv) {
    {
        ...
        int ans[]{1, 2, 3, 4, 5};
        ASSERT(std::memcmp(vec.cbegin().base(), ans, sizeof(ans)) == 0, "Fill in the correct values.");
    }
    {
        ...
        {
            vec.shrink_to_fit(); // ! 将 capacity 缩减到 size
            ASSERT(vec.capacity() == 5, "Fill in the correct value.");
            vec.clear(); // ! 清空元素不会改变 capacity
    ...
    }
    {
        ...
        ASSERT(sizeof(vec) == sizeof(std::vector<char>), "Fill in the correct value.");
        {
            ...
            vec.resize(16); // ! resize 不影响 capacity
